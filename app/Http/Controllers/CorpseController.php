...
class CorpseController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('corpses.index')->withFuneralhomes($funeralhomes)->withStations($stations)
            ->withDivisions($divisions)->withParishes($parishes);
    ...
    public function export()
    {
      ...
      //  $newExportList=array();
        ...
        //                     $corpse->pauper_burial_approved,  $corpse->buried,  $storagedays,  $excess
        //                       ];
                ...
                try {
                    ...
                    $corpse->update();
                     ...
                     try {
                        ...
                        $error_array = ['Your Info saved, Something occurred while sending Notification!'];
                       }
            ...
            foreach ($validation->messages()->getMessages() as $field_name => $messages) {
                ...
                if ( $corpse->pauper_burial_requested == 'No') {
                    ...
                    try {
                     $corpse->modified_by  = auth()->user()->id;
                    $corpse->update();
                    $success_output = '<div class="alert alert-success"> Request made Sucessfully! </div>';
        ...
        foreach ($data as $key => $value) {
            ...
            {       $search=true;
                    if ($value != '' && $count > 0 && $key != 'regNum') {
                        ...
                        if ($key == 'pauper_burial_approved' || $key == 'buried' || $key == 'division_id' ||  $key == 'anatomy' ||  $key == 'parish' || $key == 'funeralhome_id' ||  $key == 'station_id' ||   $key == 'sex' || $key == 'death_date' || $key == 'pickup_date' ||  $key == 'pauper_burial_requested_date' || $key == 'postmortem_date' || $key == 'burial_date') {
...
public function checkUniqueCrNo(Request $request){
    ...
    if ($validation->fails()) {
        ...
        $corpse = Corpse::where('cr_no' ,   $newCr_no )->pluck('cr_no');
            ...
            if( $corpse !='[]'){
                $error_array = [$corpse.' Corpse already exist!' ];
            }else{
                $success_output = '';
            }
            ...
            $error_array =['Error, Something occurred while checking!'];
        }
    ...
    public function postdata(Request $request)
    {
        ...
        if ($validation->fails()) {
           ...
           $corpse->next_of_kin_email = $request->input('next_of_kin_email');
            ...
            $corpse->manner_death = $request->input('manner_death');
            $corpse->anatomy = $request->input('anatomy');
            $corpse->condition = $request->input('condition');
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function create()
    {
        ...
        $ranks =  Rank::all();
        return view('corpses.create')->withFuneralhomes($funeralhomes)->withRanks($ranks)->withStations($stations);
    }
    ...
    public function edit($id)
    {
        ...
        //  dd($investigators );
        return view('corpses.edit')->withCorpse($corpse)->withFuneralhomes($funeralhomes)->withRanks($ranks)->withStations($stations)->withInvestigators($investigators);
    }
        ...
        foreach ($datax as $io) {
            $data .=  '<p> No. ' . $io->regNum . '  ' . $io->rank->rank . '  ' . $io->investigator_first_name . '  ' . $io->investigator_last_name . ',
               stationed at  ' . $io->station->station . ' in ' . $io->station->division->division . ' division contact #: ' .
                $io->contact_no . ' assigned date ' . $io->assign_date . ' <a  onclick="getEditInvest_id(' . $io->id . ')" href="#" class="btn editInvestment  btn-default btn-xs"><i class="glyphicon glyphicon-edit"></i> Edit</a>
                <a  onclick="deleteInvestigator(' . $io->id . ')" href="#" class="btn editInvestment  btn-default btn-xs"><i class="fa fa-trash-o" aria-hidden="true"></i>Delete</a>
             </p> <br> ';
        }
